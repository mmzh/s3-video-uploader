...
public class AwsS3Api {
    ...
    public AwsS3Api(HashMap < String, String > aws) {
        ...
        s3client = AmazonS3ClientBuilder.standard().withCredentials(new AWSStaticCredentialsProvider(new BasicAWSCredentials(this.awsKey, this.awsSecret)))
            .withRegion(this.awsRegion)
            .build();
    }
    /**
     * This method is used to transfer local file into s3 bucket.
     * @param file This is first parameter which represents local file name in String format
     * @param vinfo This is second parameter, a serialized data that contains basic video information
     * @param orinName This is third parameter, 
     ...
     * @param no parameter
     * @return ArrayList<Video>, a list contains video objects
     * @see ParseException
     */
    public ArrayList < Video > listObjects() throws ParseException {
        ...
        do {
            ...
            for (S3ObjectSummary objectSummary:
                ...
                if (vinfo.get("detail") != null) v.setDetail(vinfo.get("detail").replace("||", "; ").replace("_", " ").replace("=", ": "));
                videos.add(v);
            }
     ...
     * @param key This is first parameter, represent file name in s3 bucket
     * @return Map<String, String> , return key and value pairs of file's meta information
     */
